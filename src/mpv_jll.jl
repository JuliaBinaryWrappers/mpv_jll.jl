...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import mpv_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "mpv_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"mpv", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "mpv_jll", "src", "mpv_jll.jl"))
        @info("mpv_ll dev'ed out to /depot/dev/mpv_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("3b7eb3ab-5558-5250-ad56-7cbe1b8a43e3"))
platforms = [Pkg.Artifacts.unpack_platform(e, "mpv", artifacts_toml) for e in artifacts["mpv"]]
...
if best_platform === nothing
    @debug("Unable to load mpv; unsupported platform $(triplet(platform_key_abi()))")
else
